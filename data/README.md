...
- Run `python3 tools/validate.py` after any edit.

## Fields worth knowing

- `player_expected` — the level range a player on the critical path is likely to be in at that
  point. Boss levels must sit above the top of it; enemy bands must reach it.
...
- `no_flee` — carried by all eight bosses and by exactly one regular enemy, the Retention
  Specialist, who exists to introduce the flag before it matters.
